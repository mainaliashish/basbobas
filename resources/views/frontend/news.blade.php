										...
										<div class="inner-page-news-box">
											<div class="inner-news-box-top">
												<div class="news-published">
													<p class="news-date">{{ $n->created_at['day'] }} <br> {{ $n->created_at['month'] }}</p>
													<p class="news-year">{{ $n->created_at['year'] }}</p>
												</div>	
												<img alt="news1" src="{{ asset(imagePath($n->news_image)) }}" class="img-responsive">
											</div>
											<div class="inner-news-box-bottom">
												<h4><a href="{{ route('frontend-news-detail', $n->id) }}">{{ $n->news_title }}</a></h4>
												<p>{!! Str::limit($n->news_description, 120, '....') !!}</p>
												<a class="btn-read-more" href="{{ route('frontend-news-detail', $n->id) }}">Read More<i aria-hidden="true" class="fa fa-chevron-right"></i></a>
											</div>
										</div>
									</div>							
									@endforeach
